# 💰 NFG App - Monthly Pricing Strategy

## 🎯 Recommended Pricing Tiers

### **Tier 1: Starter** - **$99/month** ⭐ (RECOMMENDED START)
- **Target:** Small facilities management companies (1-10 users)
- **Included:**
  - Up to **5 users**
  - Up to **3 sites**
  - Core features (Jobs, Bookings, Sites, Inventory)
  - Basic reporting
  - Email support
  - PWA access
  - 5GB storage
- **Annual Plan:** $89/month (billed annually) = **$1,068/year** (10% discount)

### **Tier 2: Professional** - **$149/month** ⭐⭐ (BEST VALUE)
- **Target:** Growing companies (11-50 users)
- **Included:**
  - Up to **20 users**
  - **Unlimited sites**
  - All features (Jobs, Bookings, Sites, Inventory, Time Tracking, Billing, Messaging)
  - Advanced reporting & analytics
  - Priority email support
  - PWA + Mobile web access
  - 25GB storage
  - Bulk operations
  - Export capabilities
- **Annual Plan:** $134/month (billed annually) = **$1,608/year** (10% discount)

### **Tier 3: Enterprise** - **$599/month** ⭐⭐⭐
- **Target:** Established companies (51-200 users)
- **Included:**
  - Up to **50 users**
  - **Unlimited sites**
  - All Professional features
  - Client portal access
  - QuickBooks integration (when ready)
  - API access
  - Advanced analytics & custom reports
  - Priority phone support
  - 100GB storage
  - White-label options
  - Custom integrations
- **Annual Plan:** $539/month (billed annually) = **$6,468/year** (10% discount)

### **Tier 4: Enterprise** - **Custom Pricing** ($500-$2,000+/month)
- **Target:** Large enterprises (200+ users)
- **Included:**
  - **Unlimited users**
  - **Unlimited sites**
  - Everything in Business
  - Dedicated account manager
  - 24/7 priority support
  - Custom development
  - On-premise deployment option
  - SLA guarantees
  - Training & onboarding
  - Unlimited storage
  - Multi-company management
- **Contact for pricing**

---

## 📊 Add-On Pricing

### **Per-User Add-Ons:**
- **Additional users:** $10/user/month (after base tier limit)
- **Example:** Starter (5 users) → Add 3 more = $89 + ($10 × 3) = **$119/month**

### **Feature Add-Ons:**
- **QuickBooks Integration:** +$29/month
- **Stripe Payment Processing:** +$39/month (+ 2.9% transaction fee)
- **Advanced Analytics:** +$49/month
- **Client Portal:** +$49/month
- **API Access:** +$79/month
- **White-Label:** +$99/month
- **SMS Notifications:** +$19/month (per 1,000 texts)

### **Storage Add-Ons:**
- **Additional 10GB:** +$10/month
- **Additional 50GB:** +$40/month
- **Additional 100GB:** +$75/month

---

## 💡 Pricing Strategy Rationale

### **Why $99 Starter Tier?**

**Competitive Analysis:**
- **ServiceTitan:** $50-$200/user/month → $250-$1,000/month for 5 users
- **FieldPulse:** $89-$199/user/month → $445-$995/month for 5 users
- **Jobber:** $129-$229/month flat rate (unlimited users)
- **Housecall Pro:** $49-$169/user/month → $245-$845/month for 5 users

**Your Positioning:**
- ✅ **More affordable** than ServiceTitan/FieldPulse (per-user pricing)
- ✅ **Better value** than Jobber (you have more features)
- ✅ **Competitive** with Housecall Pro
- ✅ **Attractive** entry point at $99

### **Why $149 Professional Tier?**

- ✅ **Sweet spot** for most facilities management companies
- ✅ **20 users** covers typical team size
- ✅ **All features** = best value proposition
- ✅ **$7.45/user/month** effective rate (very competitive)
- ✅ **Anchor pricing** makes Starter look affordable

### **Why $599 Enterprise Tier?**

- ✅ **Premium** positioning for established companies
- ✅ **50 users** for growing companies
- ✅ **Advanced features** justify premium
- ✅ **$11.98/user/month** effective rate (excellent value for enterprise)
- ✅ **API access** attracts developers/agencies
- ✅ **White-label options** for resellers

---

## 🎯 Alternative Pricing Models

### **Option A: Per-User Pricing** (Not Recommended)

**Why not:**
- ❌ Higher barrier to entry
- ❌ Customers prefer predictable costs
- ❌ Your app is feature-rich, not user-heavy

**If you did it:**
- $25/user/month (minimum 3 users = $75/month)
- Would compete poorly with flat-rate competitors

### **Option B: Usage-Based Pricing** (Future Consideration)

**Potential Model:**
- Base: $49/month (up to 100 jobs/month)
- Additional: $0.50/job over limit
- Works if you have high-volume customers

**Why wait:**
- ⚠️ Complex to implement
- ⚠️ Hard to predict costs
- ⚠️ Better for established customer base

### **Option C: Freemium** (Not Recommended Initially)

**Why not:**
- ❌ No revenue initially
- ❌ Support costs for free users
- ❌ Can add later if needed

**If you did it:**
- Free: 1 user, 1 site, basic features
- Paid: Everything else

---

## 📈 Revenue Projections

### **Year 1 Conservative (50 customers):**
- 30 × Starter ($89) = $2,670/month
- 15 × Professional ($179) = $2,685/month
- 5 × Business ($299) = $1,495/month
- **Total: $6,850/month = $82,200/year**

### **Year 1 Moderate (100 customers):**
- 50 × Starter ($89) = $4,450/month
- 35 × Professional ($179) = $6,265/month
- 15 × Business ($299) = $4,485/month
- **Total: $15,200/month = $182,400/year**

### **Year 1 Optimistic (150 customers):**
- 75 × Starter ($89) = $6,675/month
- 55 × Professional ($179) = $9,845/month
- 20 × Business ($299) = $5,980/month
- **Total: $22,500/month = $270,000/year**

---

## 💰 Pricing Psychology Tips

### **1. Anchor High**
- Show Enterprise tier first (even if custom pricing)
- Makes other tiers look affordable

### **2. Highlight Value**
- "Save $240/year" on annual plans
- "Most Popular" badge on Professional
- Compare vs. competitors (save $X/user)

### **3. Feature Comparison Table**
```
| Feature           | Starter | Professional | Enterprise |
|-------------------|---------|--------------|------------|
| Users             | 5       | 20           | 50         |
| Sites             | 3       | Unlimited    | Unlimited  |
| Time Tracking     | ✅      | ✅           | ✅         |
| Billing           | ❌      | ✅           | ✅         |
| API Access        | ❌      | ❌           | ✅         |
| Price             | $99     | $149         | $599       |
| Per User/Month    | $19.80  | $7.45        | $11.98     |
```

### **4. Annual Discount**
- Offer **2 months free** on annual plans (11% discount)
- Example: "Pay annually, save $240" (on Professional)
- Increases cash flow, reduces churn

---

## 🎯 Final Recommended Pricing

### **Launch Pricing (First 100 customers):**

**Tier 1: Starter** - **$79/month** (Early Bird: 20% off)
- **Limited time offer:** First 100 customers get $20/month off
- **Regular price after:** $99/month

**Tier 2: Professional** - **$119/month** (Early Bird: 20% off)
- **Limited time offer:** First 100 customers get $30/month off
- **Regular price after:** $149/month

**Tier 3: Enterprise** - **$479/month** (Early Bird: 20% off)
- **Limited time offer:** First 100 customers get $120/month off
- **Regular price after:** $599/month

**Why Launch Discount:**
- ✅ Get initial customers quickly
- ✅ Build case studies/testimonials
- ✅ Generate early revenue
- ✅ Word-of-mouth marketing

---

## 📋 Pricing Page Example

```
╔═══════════════════════════════════════════════════════╗
║  Choose Your Plan                                    ║
╠═══════════════════════════════════════════════════════╣
║                                                       ║
║  ┌─────────────┐  ┌─────────────┐  ┌─────────────┐  ║
║  │  STARTER    │  │PROFESSIONAL │  │   BUSINESS  │  ║
║  │   $89/mo    │  │  $179/mo    │  │   $299/mo   │  ║
║  │             │  │   ⭐ POPULAR│  │             │  ║
║  │  5 users    │  │   20 users  │  │   50 users  │  ║
║  │  3 sites    │  │  Unlimited  │  │  Unlimited  │  ║
║  │             │  │             │  │             │  ║
║  │ [Get Started]│  │ [Get Started]│ │ [Get Started]│  ║
║  └─────────────┘  └─────────────┘  └─────────────┘  ║
║                                                       ║
║  💰 Save 10% with annual billing                    ║
╚═══════════════════════════════════════════════════════╝
```

---

## 🎯 My Recommendation

### **Start with these prices:**

1. **Starter: $99/month**
   - Perfect entry point
   - Competitive with market
   - Low barrier to entry

2. **Professional: $149/month** ⭐ BEST VALUE
   - Highlight as "Most Popular"
   - Best per-user value
   - Target this tier hard

3. **Enterprise: $599/month**
   - Premium positioning
   - Advanced features justify price
   - Natural upgrade path
   - White-label options available

### **Launch Strategy:**
- **First 50 customers:** 20% off forever (early adopter pricing)
- **Next 50 customers:** 10% off for first year
- **After 100:** Regular pricing

### **Annual Plans:**
- Always offer 11% discount (2 months free)
- Improves cash flow
- Reduces churn

---

## 📊 Competitor Comparison

| Platform | Pricing | Users | Your Price | Your Value |
|----------|---------|-------|------------|------------|
| **ServiceTitan** | $50-$200/user | 5 users = $250-$1,000 | $99 | **60-90% cheaper** |
| **FieldPulse** | $89-$199/user | 5 users = $445-$995 | $99 | **78-90% cheaper** |
| **Jobber** | $129-$229/mo | Unlimited | $99 | **23-57% cheaper** |
| **Housecall Pro** | $49-$169/user | 5 users = $245-$845 | $99 | **10-60% cheaper** |
| **FieldEdge** | $150-$250/user | 5 users = $750-$1,250 | $99 | **87-92% cheaper** |

**✅ Your pricing is VERY competitive!**

---

## 💵 Quick Pricing Summary

### **Recommended Monthly Prices:**

| Tier | Monthly | Annual (10% off) | Best For |
|------|---------|------------------|----------|
| **Starter** | **$99** | **$89/mo** ($1,068/yr) | Small teams (1-10 users) |
| **Professional** | **$149** | **$134/mo** ($1,608/yr) | Growing teams (11-50 users) |
| **Enterprise** | **$599** | **$539/mo** ($6,468/yr) | Established companies (51-200 users) |

### **Add-Ons:**
- Additional users: **$10/user/month**
- QuickBooks: **+$29/month**
- Payment Processing: **+$39/month**
- API Access: **+$79/month**

---

## 🎯 Final Answer

**Price your app at:**

1. **$99/month** - Starter (5 users, 3 sites)
2. **$149/month** - Professional (20 users, unlimited) ⭐ **Target this**
3. **$599/month** - Enterprise (50 users, all features)

**This pricing is:**
- ✅ **Competitive** with market leaders
- ✅ **Affordable** for target market
- ✅ **Profitable** (good margins)
- ✅ **Scalable** (multiple tiers)
- ✅ **Flexible** (add-ons available)

**Start with these prices and adjust based on:**
- Customer feedback
- Conversion rates
- Competitor moves
- Market demand


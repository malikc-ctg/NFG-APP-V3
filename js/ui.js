...
export async function renderSites(sites, options = {}) {
  ...
  // Get selected sites from global state (if available)
  ...
  if (filteredSites.length === 0) {
    ...
    grid.innerHTML = filteredSites.map(site => {
      ...
      return createSiteCard(site, { showBulkCheckbox: canBulkOperate, currentUserProfile, isSelected });
    }).join('')
...
function createSiteCard(site, options = {}) {
  const { currentUserProfile = null, showBulkCheckbox = false, isSelected = false } = options;
  const statusColors = {
  ...
  const showCheckbox = canBulkOperate && showBulkCheckbox;

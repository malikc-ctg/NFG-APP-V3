...
// Fetch and display low stock items widget
async function fetchLowStockWidget() {
  try {
    ...
    try {
      ...
      console.warn('[Low Stock Widget] View not available, using fallback');
      const result = await supabase
        .from('site_inventory')
        .select(`
          quantity,
      ...
      if (data && !error) {
        data = data.map(item => {
          const threshold = item.inventory_items?.low_stock_threshold || 0;
          ...
          return {
            ...
            stock_status: qty === 0 ? 'out' : qty < threshold ? 'low' : 'ok'
          };
        }).filter(item => item.stock_status === 'low' || item.stock_status === 'out');
      }
    }
    ...
    if (!data || data.length === 0) {
      widget.innerHTML = `
        <div class="py-4 text-center text-gray-500 dark:text-gray-400">
          ...
          <p class="text-sm font-medium">All items in stock</p>
        </div>
      `;
      if (window.lucide) lucide.createIcons();
      return;
    ...
    widget.innerHTML = data.map(item => {
      const statusColor = item.stock_status === 'out' 
        ? 'text-red-600 dark:text-red-400' 
        : 'text-orange-600 dark:text-orange-400';
      const statusIcon = item.stock_status === 'out' ? 'package-x' : 'alert-triangle';
      
      return `
        <div class="py-2 px-2 rounded-lg hover:bg-nfglight/50 dark:hover:bg-gray-700/50 transition cursor-pointer border-l-2 ${item.stock_status === 'out' ? 'border-red-200 dark:border-red-800' : 'border-orange-200 dark:border-orange-800'}" 
             onclick="window.location.href='inventory.html'">
          <div class="flex items-start justify-between gap-2">
            <div class="flex-1 min-w-0">
              <p class="font-medium text-sm text-nfgblue dark:text-blue-400 truncate">${item.item_name || 'Unknown Item'}</p>
              <p class="text-xs text-gray-500 dark:text-gray-400">${item.site_name || 'Unknown Site'}</p>
            </div>
            <div class="flex items-center gap-1.5 flex-shrink-0">
              <i data-lucide="${statusIcon}" class="w-4 h-4 ${statusColor}"></i>
              <span class="text-xs font-medium ${statusColor}">${item.quantity} ${item.unit || ''}</span>
            </div>
          </div>
        </div>
      `;
    ...
    console.error('Error fetching low stock widget:', error);
    const widget = document.getElementById('low-stock-widget');
    if (widget) {
      widget.innerHTML = `
        <div class="py-4 text-center text-gray-500 dark:text-gray-400 text-sm">
          <p>Unable to load low stock items</p>
        </div>
      `;
    }
  }

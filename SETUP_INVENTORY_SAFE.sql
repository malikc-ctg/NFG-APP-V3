-- ============================================
-- NFG Inventory Tracking System (SAFE VERSION)
-- ============================================
-- This version uses "IF NOT EXISTS" to prevent errors
...
CREATE INDEX IF NOT EXISTS idx_site_inventory_site ON site_inventory(site_id);
CREATE INDEX IF NOT EXISTS idx_site_inventory_item ON site_inventory(item_id);
...
DO $$
BEGIN
  -- Only insert if no items exist yet
...
BEGIN
  ...
  RAISE NOTICE '🌱 Sample data: 7 categories, 23 items';
  RAISE NOTICE '';
  RAISE NOTICE '🎯 Ready to use! Go to inventory.html';
END $$;
